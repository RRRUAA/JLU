...
(function () {
    ...
    WdatePicker = main;
    var w = window, emptyEl = {innerHTML: ''}, d = 'document', de = 'documentElement', tag = 'getElementsByTagName',
    ...
    if (appName == 'Microsoft Internet Explorer') $IE = true; else if (appName == 'Opera') $OPERA = true; else $FF = true;
    ...
    function getBound(o, ignoreScr) {
        if (o.getBoundingClientRect) {
            ...
            var patterns = {ROOT_TAG: /^body|html$/i, OP_SCROLL: /^(?:inline|table-row)$/i};
            ...
            if (parentNode != o) {
                while (parentNode) {
                    ...
                    if (getStyle(parentNode, 'position').toLowerCase() == 'fixed') hssFixed = true; else if (parentNode.tagName.toLowerCase() == "body") win = parentNode.ownerDocument.defaultView;
    ...
    function display(obj, value) {
        if (obj) {
            if (value != null) obj.style.display = value; else return getStyle(obj, 'display')
        }
    }
    ...
    function showPicker(cfg, preLoad) {
        ...
        function ddInit(cont, cfg) {
            var dm = dptop[d].domain, isCross = false,
            ...
            function ddWrite() {
                var realLang = cfg.getRealLang(), ver = '4.9.0b3';
        ...
        function setPos(dp) {
            ...
            var objxy = getBound(el), mm = getAbsM(w), currWH = getWH(dptop), scr = getScroll(dptop),
                ddHeight = $dp.dd.offsetHeight, ddWidth = $dp.dd.offsetWidth;
            if (isNaN(t)) t = 0;
