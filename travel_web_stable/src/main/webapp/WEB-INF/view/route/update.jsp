							...
							<div class="col-5">
								<form id="saveForm" enctype="multipart/form-data">
									...
									<div class="form-group">
										<label>线路编号</label> <input type="text" name="route_no"
																	   value="${data.route_no }"
									...
									<div class="form-group">
										<label>线路名称</label> <input type="text" name="name" value="${data.name }"
																	   class="form-control"> <small
											class="form-text text-muted"></small>
									</div>

									<div class="form-group">
										<label>出发地</label> <input type="text" name="start_spot"
																	 value="${data.start_spot }"
																	 class="form-control"> <small
											class="form-text text-muted"></small>
									</div>

									<div class="form-group">
										<label>目的地</label> <input type="text" name="end_spot"
																	 value="${data.end_spot }"
																	 class="form-control"> <small
											class="form-text text-muted"></small>
									</div>
...
<script>
    $("#save").click(function () {
        $.ajax({
            cache: true,
            type: "post",
            url: "RouteServlet?action=updateRoute",
            data: $("#saveForm").serialize(),
            async: false,
            success: function (e) {
                if (e == 'yes') {
                    alert("修改成功！");
                    window.parent.location.href = "RouteServlet?action=RouteList";
                } else {
                    alert("修改失败！");
                }
            }
