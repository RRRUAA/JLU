	...
	<main>
		...
		<div id="body">
			<main>
				<div class="main">
					<div class="search">
						<form class="form-inline float-left" action="RouteServlet?action=RoomList"
							  method="post">
					...
					<div>
						<table class="table">
							<thead>
							<tr>
								<th scope="col">路线编号</th>
								<th scope="col">路线名称</th>
								<th scope="col">路线描述</th>
								<th scope="col">发布人</th>
								<th scope="col">起始地</th>
								<th scope="col">目的地</th>
								...
								<tr>
									...
									<td>
										...
										<a class="btn btn-primary btn-sm"
										   href="RouteServlet?action=toUpdateRoute&id=${data.id }">修改</a> <a
					...
					<div class="page">
						<ul class="pagination">
							<li class="page-item"><a class="page-link"
													 href="RouteServlet?action=RouteList&p=1">首页</a></li>
							<c:if test="${cp>1}">
								<li class="page-item"><a class="page-link"
														 href="RouteServlet?action=RouteList&p=${cp-1}">上一页</a></li>
							</c:if>
							<c:forEach begin="${cp-2>1 ? (cp-2) :1}"
								...
								<c:if test="${cp==e}">
									<li class="page-item"><a class="page-link"
															 ...
															 href="RouteServlet?action=RouteList&p=${e}">${e}</a></li>
								</c:if>
								<c:if test="${cp!=e}">
									<li class="page-item"><a class="page-link"
															 href="RouteServlet?action=RouteList&p=${e}">${e}</a></li>
							...
							<c:if test="${cp<tp}">
								<li class="page-item"><a class="page-link"
														 href="RouteServlet?action=RouteList&p=${cp+1}">下一页</a></li>
							</c:if>
							<li class="page-item"><a class="page-link"
													 href="RouteServlet?action=RouteList&p=${tp}">尾页</a></li>
						</ul>
...
<script>
    ...
    $('a.delete').click(function (e) {
        ...
        if (confirm("确定删除吗?")) {
            $.ajax({
                cache: true,
                url: "RouteServlet?action=deleteRoute",
                data: {"id": id},
                type: "post",
                async: false,
