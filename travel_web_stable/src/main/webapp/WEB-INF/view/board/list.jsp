...
<head>
	<meta charset="UTF-8">
	<meta name="viewport"
		  content="width=device-width, Board-scalable=no, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0">
	...
	<main>
		...
		<div id="body">
			<main>
				<div class="main">
					<div class="search">
						<form class="form-inline float-left" action="BoardServlet?action=BoardList"
							  method="post">
						...
						<!-- <a class="btn btn-primary float-right" href="Board_create.html"><i
								...
								<tr>
									...
									<td>
										...
										<a class="btn btn-primary btn-sm"
										   href="BoardServlet?action=toUpdateBoard&id=${data.id }">修改</a> <a
					...
					<div class="page">
						<ul class="pagination">
							<li class="page-item"><a class="page-link"
													 href="BoardServlet?action=BoardList&p=1">首页</a></li>
							<c:if test="${cp>1}">
								<li class="page-item"><a class="page-link"
														 href="BoardServlet?action=BoardList&p=${cp-1}">上一页</a></li>
							</c:if>
							<c:forEach begin="${cp-2>1 ? (cp-2) :1}"
								...
								<c:if test="${cp==e}">
									<li class="page-item"><a class="page-link"
															 ...
															 href="BoardServlet?action=BoardList&p=${e}">${e}</a></li>
								</c:if>
								<c:if test="${cp!=e}">
									<li class="page-item"><a class="page-link"
															 href="BoardServlet?action=BoardList&p=${e}">${e}</a></li>
							...
							<c:if test="${cp<tp}">
								<li class="page-item"><a class="page-link"
														 href="BoardServlet?action=BoardList&p=${cp+1}">下一页</a></li>
							</c:if>
							<li class="page-item"><a class="page-link"
													 href="BoardServlet?action=BoardList&p=${tp}">尾页</a></li>
						</ul>
...
<script>
    ...
    $('a.delete').click(function (e) {
        ...
        if (confirm("确定删除吗?")) {
            $.ajax({
                cache: true,
                url: "BoardServlet?action=deleteBoard",
                data: {"id": id},
                type: "post",
                async: false,
