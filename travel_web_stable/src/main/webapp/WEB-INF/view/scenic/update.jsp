			...
			<main>
				<div class="main">
							...
							<div class="col-5">
								<form id="saveForm">
									...
									<div class="form-group">
										<label>景点名称</label> <input type="text" name="names" value="${data.names }"
									...
									<div class="form-group">
										...
										<select name="level" class="form-control">
											<c:forEach items="${categories }" var="row">
												<option value="${row.id }"
														<c:if test="${row.id== data.c_id}">selected</c:if>>${row.cname }</option>
											</c:forEach>
										</select>
										<small
												class="form-text text-muted"></small>
					...
					<div class="submit-box">
						...
						<a type="button" href="ScenicServlet?action=ScenicList"
...
<script>
    $("#save").click(function () {
        $.ajax({
            cache: true,
            type: "post",
            url: "ScenicServlet?action=updateScenic",
            data: $("#saveForm").serialize(),
            async: false,
            success: function (e) {
                if (e == 'yes') {
                    alert("修改成功！");
                    window.parent.location.href = "ScenicServlet?action=ScenicList";
                } else {
                    alert("修改失败！");
                }
            }
