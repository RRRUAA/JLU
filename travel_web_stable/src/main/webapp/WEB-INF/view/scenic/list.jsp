	...
	<main>
		...
		<div id="body">
			<main>
				<div class="main">
					<div class="search">
						<form class="form-inline float-left" action="ScenicServlet?action=ScenicList"
							  method="post">
							<div class="form-group">
								<input type="text" name="key" value="${key}"
									   class="form-control" placeholder="景点名称">
							</div>
						...
						<table class="table">
							...
							<c:forEach items="${list}" var="data">
								<tr>
									...
									<td><img src="<%=path %>/upload/${data.photo}" style="width:250px; height:90px; ">
									</td>
									<td>
										<input id="${data.id}" value="${data.id}" type="hidden" class="weui-input"/>
										<a class="btn btn-primary btn-sm"
										   href="ScenicServlet?action=toUpdateScenic&id=${data.id }">修改</a> <a
					...
					<div class="page">
						<ul class="pagination">
							<li class="page-item"><a class="page-link"
													 href="ScenicServlet?action=ScenicList&p=1">首页</a></li>
							<c:if test="${cp>1}">
								<li class="page-item"><a class="page-link"
														 href="ScenicServlet?action=ScenicList&p=${cp-1}">上一页</a>
								</li>
							</c:if>
							<c:forEach begin="${cp-2>1 ? (cp-2) :1}"
								...
								<c:if test="${cp==e}">
									<li class="page-item"><a class="page-link"
															 ...
															 href="ScenicServlet?action=ScenicList&p=${e}">${e}</a></li>
								</c:if>
								<c:if test="${cp!=e}">
									<li class="page-item"><a class="page-link"
															 href="ScenicServlet?action=ScenicList&p=${e}">${e}</a></li>
							...
							<c:if test="${cp<tp}">
								<li class="page-item"><a class="page-link"
														 href="ScenicServlet?action=ScenicList&p=${cp+1}">下一页</a>
								</li>
							</c:if>
							<li class="page-item"><a class="page-link"
													 href="ScenicServlet?action=ScenicList&p=${tp}">尾页</a></li>
						</ul>
...
<script>
    ...
    $('a.delete').click(function (e) {
        ...
        if (confirm("确定删除吗?")) {
            $.ajax({
                cache: true,
                url: "ScenicServlet?action=deleteScenic",
                data: {"id": id},
                type: "post",
                async: false,
