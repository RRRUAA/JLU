...
<div class="logo-phone">
	<div class="width1190">
		<h3 class="logo" style="font-size:20px"><a href="IndexServlet?action=toIndex">旅游景点服务平台</a></h3>
	...
	<div class="content">
		...
		<div class="width1190">
			...
			<h2 class="title">景点门票 <!-- <a href="CommonServlet?action=moreHouse">更多&gt;&gt;</a> --></h2>
			<div class="index-fang-list">
				<c:forEach items="${ticketList}" var="data">
					<dl>
						<dt><a href="IndexServlet?action=toScenic&id=${data.id }"><img
								src="upload/${data.scenic.photo }" width="286" height="157"/></a></dt>
						<dd>
							<h4><a href="IndexServlet?action=toScenic&id=${data.id }">名称：${data.scenic.names  }</a>
							</h4>
							<div class="hui">价格：${data.price }<span style="color:red">￥</span></div>
							<div class="hui">等级：${data.level } <a href="IndexServlet?action=toScenic&id=${data.id }"
																	style="color:red;">购票</a></div>
							<a href="IndexServlet?action=toScenic&id=${data.id }" style="color:blue">查看详情</a>
							<br>
						</dd>
			...
			<div class="page" style="text-align:center;margin-top:100px;">
				...
				<ul class="pagination">
					<li class="page-item"><a class="page-link"
											 href="IndexServlet?action=toIndex&p=1">首页</a></li>
					<c:if test="${cp>1}">
						<li class="page-item"><a class="page-link"
												 href="IndexServlet?action=toIndex&p=${cp-1}">上一页</a></li>
					</c:if>
					<c:forEach begin="${cp-2>1 ? (cp-2) :1}"
						...
						<c:if test="${cp==e}">
							<li class="page-item"><a class="page-link" style="background-color: #007bff;color:white"
													 href="IndexServlet?action=toIndex&p=${e}">${e}</a></li>
						</c:if>
						<c:if test="${cp!=e}">
							<li class="page-item"><a class="page-link"
													 href="IndexServlet?action=toIndex&p=${e}">${e}</a></li>
					...
					<c:if test="${cp<tp}">
						<li class="page-item"><a class="page-link"
												 href="IndexServlet?action=toIndex&p=${cp+1}">下一页</a></li>
					</c:if>
					<li class="page-item"><a class="page-link"
