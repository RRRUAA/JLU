...
<head>
	<meta charset="UTF-8">
	<meta name="viewport"
		  content="width=device-width, Room-scalable=no, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0">
	...
	<main>
		...
		<div id="body">
			<main>
				<div class="main">
					<div class="search">
						<form class="form-inline float-left" action="RoomServlet?action=RoomList"
							  method="post">
						...
						<!-- <a class="btn btn-primary float-right" href="Room_create.html"><i
						...
						<table class="table">
							...
							<c:forEach items="${list}" var="data">
								<tr>
									<td>${data.title }</td>
									<td>${data.name }</td>
									<td><img src="/upload/${data.photo}" style="width:250px; height:90px; "></td>
									...
									<td>
										...
										<a class="btn btn-primary btn-sm"
										   href="RoomServlet?action=toUpdateRoom&id=${data.id }">修改</a> <a
					...
					<div class="page">
						<ul class="pagination">
							<li class="page-item"><a class="page-link"
													 href="RoomServlet?action=RoomList&p=1">首页</a></li>
							<c:if test="${cp>1}">
								<li class="page-item"><a class="page-link"
														 href="RoomServlet?action=RoomList&p=${cp-1}">上一页</a></li>
							</c:if>
							<c:forEach begin="${cp-2>1 ? (cp-2) :1}"
								...
								<c:if test="${cp==e}">
									<li class="page-item"><a class="page-link"
															 ...
															 href="RoomServlet?action=RoomList&p=${e}">${e}</a></li>
								</c:if>
								<c:if test="${cp!=e}">
									<li class="page-item"><a class="page-link"
															 href="RoomServlet?action=RoomList&p=${e}">${e}</a></li>
							...
							<c:if test="${cp<tp}">
								<li class="page-item"><a class="page-link"
														 href="RoomServlet?action=RoomList&p=${cp+1}">下一页</a></li>
							</c:if>
							<li class="page-item"><a class="page-link"
													 href="RoomServlet?action=RoomList&p=${tp}">尾页</a></li>
						</ul>
...
<script>
    ...
    $('a.delete').click(function (e) {
        ...
        if (confirm("确定删除吗?")) {
            $.ajax({
                cache: true,
                url: "RoomServlet?action=deleteRoom",
                data: {"id": id},
                type: "post",
                async: false,
