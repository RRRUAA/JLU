			...
			<main>
				<div class="main">
							...
							<div class="col-5">
								<form id="saveForm">
									...
									<div class="form-group">
										<label>景点资讯主题</label> <input type="text" name="title"
																		   value="${data.title }"
																		   class="form-control"> <small
											class="form-text text-muted"></small>
									</div>
									<div class="form-group">
										<label>作者</label> <input type="text" name="editor" value="${data.editor }"
																   class="form-control"> <small
											class="form-text text-muted"></small>
									</div>
									<div class="form-group">
					...
					<div class="submit-box">
						...
						<a type="button" href="NewsServlet?action=NewsList" class="btn btn-outline-secondary">返回</a>
					</div>
				</div>
			</main>
		</div>
...
<script>
    $("#save").click(function () {
        $.ajax({
            cache: true,
            type: "post",
            url: "NewsServlet?action=updateNews",
            data: $("#saveForm").serialize(),
            async: false,
            success: function (e) {
                if (e == 'yes') {
                    alert("修改成功！");
                    window.parent.location.href = "NewsServlet?action=NewsList";
                } else {
                    alert("修改失败！");
                }
            }
