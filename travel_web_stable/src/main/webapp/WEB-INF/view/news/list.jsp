	...
	<main>
		...
		<div id="body">
			<main>
				<div class="main">
					<div class="search">
						<form class="form-inline float-left" action="NewsServlet?action=NewsList"
							  method="post">
							<div class="form-group">
								<input type="text" name="key" value="${key}"
									   class="form-control" placeholder="景点资讯主题">
							</div>
						...
						<!-- <a class="btn btn-primary float-right" href="News_create.html"><i
								...
								<tr>
									...
									<td>
										...
										<a class="btn btn-primary btn-sm"
										   href="NewsServlet?action=toUpdateNews&id=${data.id }">修改</a> <a
					...
					<div class="page">
						<ul class="pagination">
							<li class="page-item"><a class="page-link"
													 href="NewsServlet?action=NewsList&p=1">首页</a></li>
							<c:if test="${cp>1}">
								<li class="page-item"><a class="page-link"
														 href="NewsServlet?action=NewsList&p=${cp-1}">上一页</a></li>
							</c:if>
							<c:forEach begin="${cp-2>1 ? (cp-2) :1}"
								...
								<c:if test="${cp==e}">
									<li class="page-item"><a class="page-link"
															 ...
															 href="NewsServlet?action=NewsList&p=${e}">${e}</a></li>
								</c:if>
								<c:if test="${cp!=e}">
									<li class="page-item"><a class="page-link"
															 href="NewsServlet?action=NewsList&p=${e}">${e}</a></li>
							...
							<c:if test="${cp<tp}">
								<li class="page-item"><a class="page-link"
														 href="NewsServlet?action=NewsList&p=${cp+1}">下一页</a></li>
							</c:if>
							<li class="page-item"><a class="page-link"
													 href="NewsServlet?action=NewsList&p=${tp}">尾页</a></li>
						</ul>
...
<script>
    ...
    $('a.delete').click(function (e) {
        ...
        if (confirm("确定删除吗?")) {
            $.ajax({
                cache: true,
                url: "NewsServlet?action=deleteNews",
                data: {"id": id},
                type: "post",
                async: false,
