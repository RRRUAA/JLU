	...
	<main>
		...
		<div id="body">
			<main>
				<div class="main">
					<div class="search">
						<form class="form-inline float-left" action="OrdersServlet?action=OrdersList"
							  method="post">
					...
					<div>
						<table class="table">
							...
							<tr>
								...
								<th scope="col">景点图片</th>
								<th scope="col">是否评价</th>
								<th scope="col">操作</th>
							</tr>
							</thead>
							<tbody>
							...
							<c:forEach items="${list}" var="data">
								<tr>
									...
									<td>${data.ticket.scenic.names }</td>
									<td><img src="upload/${data.ticket.scenic.photo}" id="preview_img"
											 style="width:150px; height:90px; "></td>
									<td><c:if test="${data.status==1 }">已经评价</c:if>
										<c:if test="${data.status==0 }">未评价</c:if>
									</td>
									<td>
					...
					<div class="page">
						<ul class="pagination">
							<li class="page-item"><a class="page-link"
													 href="OrdersServlet?action=OrdersList&p=1">首页</a></li>
							<c:if test="${cp>1}">
								<li class="page-item"><a class="page-link"
														 href="OrdersServlet?action=OrdersList&p=${cp-1}">上一页</a>
								</li>
							</c:if>
							<c:forEach begin="${cp-2>1 ? (cp-2) :1}"
								...
								<c:if test="${cp==e}">
									<li class="page-item"><a class="page-link"
															 ...
															 href="OrdersServlet?action=OrdersList&p=${e}">${e}</a></li>
								</c:if>
								<c:if test="${cp!=e}">
									<li class="page-item"><a class="page-link"
															 href="OrdersServlet?action=OrdersList&p=${e}">${e}</a></li>
							...
							<c:if test="${cp<tp}">
								<li class="page-item"><a class="page-link"
														 href="OrdersServlet?action=OrdersList&p=${cp+1}">下一页</a>
								</li>
							</c:if>
							<li class="page-item"><a class="page-link"
													 href="OrdersServlet?action=OrdersList&p=${tp}">尾页</a></li>
						</ul>
...
<script>
    ...
    $('a.delete').click(function (e) {
        ...
        if (confirm("确定删除吗?")) {
            $.ajax({
                cache: true,
                url: "OrdersServlet?action=deleteOrders",
                data: {"id": id},
                type: "post",
                async: false,
